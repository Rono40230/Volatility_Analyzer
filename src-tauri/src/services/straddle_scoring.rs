...
impl StraddleScoreCalculator {
    ...
    pub fn calculate(
        ...
        let volatility = Self::calculate_volatility_score(candles_before, candles_after, point_value);
        let directionality = Self::calculate_directionality_score(candles_after);
        let whipsaw = Self::calculate_whipsaw_risk(candles_after, point_value);
        ...
        let whipsaw_penalty = if whipsaw > 60.0 { 0.1 } else if whipsaw > 40.0 { 0.5 } else { 1.0 };
    ...
    fn calculate_volatility_score(before: &[Candle], after: &[Candle], _point_value: f64) -> f64 {
        let atr_before = Self::calculate_atr(before);
        let atr_after = Self::calculate_atr(after);
        
        if atr_before == 0.0 { return 0.0; }
    ...
    fn calculate_directionality_score(after: &[Candle]) -> f64 {
        // Moyenne du ratio Body/Range sur les bougies après l'événement
        let mut total_ratio = 0.0;
        let mut count = 0;
        
        for c in after.iter().take(5) { // On regarde les 5 premières minutes (réaction immédiate)
    ...
    fn calculate_whipsaw_risk(after: &[Candle], _point_value: f64) -> f64 {
        ...
        for c in after.iter().take(5) {
            ...
            if range > 0.0 {
                ...
                if upper_wick > range * 0.2 && lower_wick > range * 0.2 {
                    ...
                    if risk > max_whipsaw { max_whipsaw = risk; }
                }
            }
        }
