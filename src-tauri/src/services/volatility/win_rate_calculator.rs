...
pub fn simulate_straddle_win_rate(candles: &[Candle], offset_pips: f64) -> WinRateResult {
    ...
    for i in 0..candles.len() - 15 {
        ...
        let trade_result = if buy_triggered && sell_triggered {
            ...
            let max_price = follow_up_candles.iter().map(|c| c.high).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
            ...
            let min_price = follow_up_candles.iter().map(|c| c.low).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
