...
#[derive(Debug, Serialize, Deserialize)]
pub struct ImpactGroupSummary {
    pub description: String,
    pub impact: String,
    pub count: i64,
}

pub fn get_country_name(symbol: &str) -> String {
    match symbol {
        "USD" => "États-Unis",
        "EUR" => "Zone Euro",
        "GBP" => "Royaume-Uni",
...
pub fn list_rare_events_helper(conn: &mut SqliteConnection, min_occurrences: i64, allowed_ids: &[i32]) -> Result<Vec<RareEventSummary>, String> {
...
pub fn delete_rare_events_helper(conn: &mut SqliteConnection, min_occurrences: i64, allowed_ids: &[i32]) -> Result<usize, String> {
    use crate::schema::calendar_events::dsl::*;
...
pub fn list_currencies_helper(conn: &mut SqliteConnection, allowed_ids: &[i32]) -> Result<Vec<CurrencySummary>, String> {
    use crate::schema::calendar_events::dsl::*;
    ...
    let mut currencies: Vec<CurrencySummary> = results
        .into_iter()
        .map(|(sym, count)| {
            let country = get_country_name(&sym);
            CurrencySummary {
...
pub fn list_orphan_events_helper(conn: &mut SqliteConnection, allowed_ids: &[i32]) -> Result<Vec<OrphanEventSummary>, String> {
    use crate::schema::calendar_events::dsl::*;
...
pub fn list_impact_groups_helper(conn: &mut SqliteConnection, allowed_ids: &[i32]) -> Result<Vec<ImpactGroupSummary>, String> {
    use crate::schema::calendar_events::dsl::*;

    let results = calendar_events
        .filter(calendar_import_id.eq_any(allowed_ids))
        .group_by((description, impact))
        .select((description, impact, diesel::dsl::count(id)))
        .load::<(String, String, i64)>(conn)
        .map_err(|e| format!("Database error: {}", e))?;

    let impacts: Vec<ImpactGroupSummary> = results
        .into_iter()
        .map(|(desc, imp, count)| ImpactGroupSummary {
            description: desc,
            impact: imp,
            count,
        })
...
pub fn delete_orphan_events_helper(conn: &mut SqliteConnection, allowed_ids: &[i32]) -> Result<usize, String> {
    use crate::schema::calendar_events::dsl::*;
